    ...
    */
    async resolve(params: {
        ...
        try{
            ...
            const cachedId = await this.redisClient.get(cacheKey);
            if (cachedId) {
            ...
            if (existing.length > 0) {
                ...
                await this.redisClient.setEx(cacheKey, this.cacheTtlSeconds, conversation.id);
                return conversation;
            }
            ...
            // 4) Cache write
            await this.redisClient.setEx(cacheKey, this.cacheTtlSeconds, created.id);
